  ...
  <div class="search">
    ...
    <Modal :title="dictModalTitle" v-model="dictModalVisible" :mask-closable="false" :width="500">
      <Form ref="dictForm" :model="dictForm" :label-width="85" :rules="dictFormValidate">
        ...
        <FormItem label="字典类型" prop="dict_code" class="block-tool">
          <Tooltip placement="right" :max-width="220" transfer content="建议英文名且需唯一 非开发人员谨慎修改">
            <Input v-model="dictForm.dict_code" />
          </Tooltip>
        </FormItem>
        ...
        <FormItem label="排序值" prop="seq">
          <Tooltip trigger="hover" placement="right" content="值越小越靠前，支持小数">
            <InputNumber :max="1000" :min="0" v-model="dictForm.seq"></InputNumber>
          </Tooltip>
        </FormItem>
      </Form>
      <div slot="footer">
...
export default {
  ...
  data () {
    return {
      ...
      dictFormValidate: {
        ...
        dict_code: [{ required: true, message: '不能为空', trigger: 'blur' }]
      },
      formValidate: {
        // 表单验证规则
      ...
      columns: [
        ...
        { title: '数据值', key: 'code', width: 150 },
        { title: '备注', key: 'remark', width: 150 },
        { title: '排序值', key: 'seq', width: 100, sortable: true, sortType: 'asc' },
        { title: '状态',
          key: 'status',
          width: 130,
          render: (h, params) => {
  ...
  methods: {
    ...
    search () {
      ...
      if (this.searchKey) {
        this.treeLoading = true
        getlstDict({ search_text: this.searchKey }).then(res => {
          this.treeLoading = false
          if (res.status) {
            this.treeData = res.data
          }
    ...
    selectTree (v) {
      if (v.length > 0) {
        ...
        this.dictForm = data
        this.editTitle = data.title + '(' + data.dict_code + ')'
        // 重新加载表
    ...
    getDataList () {
      ...
      if (this.selectNode.id) {
        this.searchForm.dict_code = this.selectNode.dict_code
      } else {
        delete this.searchForm.dict_code
      }
      // if (!this.searchForm.dict_code) {
      //   this.searchForm.dict_code = "";
      // }
      getlstDictDetail(this.searchForm).then(res => {
        this.loading = false
        if (res.status) {
    ...
    delDcit () {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          // 删除
          delDict({ id: this.selectNode.id }).then(res => {
            this.$Modal.remove()
            if (res.status) {
      ...
      this.$refs.dictForm.validate(valid => {
        if (valid) {
          ...
          if (this.modalType == 0) {
            ...
            addEditDict(this.dictForm).then(res => {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.modalType == 0) {
            // 添加 避免编辑后传入id等数据 记得删除
            delete this.form.id
            this.form.dict_code = this.selectNode.dict_code
            addEditDictDetail(this.form).then(res => {
              this.submitLoading = false
              if (res.status) {
                this.$Message.success('操作成功')
                this.getDataList()
    ...
    remove (v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          // 删除
          delDictDetail({ id: v.id }).then(res => {
            this.$Modal.remove()
            if (res.status) {
              this.$Message.success('操作成功')
              this.getDataList()
